...
const DEFAULT_SYSTEM_PROMPT = [
  'You are Moxy, answering through the MoxyTalks browser extension.',
...
async function requestStudio(method, path, body) {
  ...
  throw lastError || new Error('Moxy is not reachable');
}
...
sendBtn.addEventListener('click', async () => {
  ...
  try {
    ...
    if (!response?.ok) {
      throw new Error(response?.error || 'Moxy did not accept the clip');
    }
    statusEl.textContent = 'Sent to Moxy.';
  } catch (error) {
...
async function runAsk({ forceAct = false } = {}) {
  ...
  try {
    ...
    if (!health?.loaded_model) {
      throw new Error('No model loaded. Load a model in MLX-Moxy-Wirks first.');
    }
    ...
    if (!result?.response) {
      throw new Error(result?.error || 'No response from Moxy');
    }
